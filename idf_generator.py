...
# Passar as variaveis numericas pra float soh pra garantir
	...
	wwr = float(wwr)
	tamanhoprojecao = float(tamanhoprojecao)
...
#variaveis dependentes
	...
	DormWidth = bwidth/3
	DormLength = blength/2
	SalaLength = blength-DormLength
	#SalaWidth = bwidth
	if DormWidth < lPorta: #garante que a porta cabe na parede
		lPorta = DormWidth*.999
	x0Porta = (DormWidth/2)-(lPorta/2)
	
	if pedireito*hjan <= pedireito*wwr: #garante que a area de janela seja compativel com a area de fachada
		zBase = pedireito*(1-wwr)/2
		zTopo = zBase+(pedireito*wwr)
		ljan1Dorm = DormWidth*.001
		ljan2Dorm = DormWidth*.999
		ljan1bwidth = bwidth*.001
		ljan2bwidth = bwidth*.999
		ljan1SalaLength = SalaLength*.001 # SalaLength = DormLength
		ljan2SalaLength = SalaLength*.999
	else:
		zBase = pedireito*(1-hjan)/2
		zTopo = zBase+(pedireito*hjan)
		ljan1Dorm = (DormWidth/2)-((DormWidth*wwr)/hjan)/2
		ljan2Dorm = ljan1Dorm+((DormWidth*wwr)/hjan)
		ljan1bwidth = (bwidth/2)-((bwidth*wwr)/hjan)/2
		ljan2bwidth = ljan1bwidth+((bwidth*wwr)/hjan)
		ljan1SalaLength = (SalaLength/2)-((SalaLength*wwr)/hjan)/2
		ljan2SalaLength = ljan1SalaLength+((SalaLength*wwr)/hjan)
		
	#Lowrise ou HighRise
	
	if uh == 'bot' or uh == 'mid' or uh == 'top' or uh == 'piu' or uh == 'pim':
	...
	if blengthratio > 1:
		...
		bldgRatio = blengthratio**(-1)
	else:
		bldgRatio = blengthratio

	#Lista das Zonas

	Zonas = dict()
	Zonas['Name'] = ['Dorm1','Dorm2','Dorm3','Sala']

	#x,y,z das Zonas
	Zonas['X'] = [0,DormWidth,2*DormWidth,0]
	Zonas['Y'] = [SalaLength,SalaLength,SalaLength,0]
	Zonas['Z'] = [hpav,hpav,hpav,hpav]
	...
	if uh == 'bot' or uh == 'uni' or uh == 'piu' or uh == 'pim':
  ...
  ########################################################## Rayner
	...
	'''	for i in range(len(Zonas['Name'])):
		...
		obj['Zone Name'] = Zonas['Name'][i]
		idf.add(obj)'''
		

	for i in range(len(FenSurface['Name'])):
		obj = IDF._create_datadict('AirflowNetwork:MultiZone:Surface')
		obj['Surface Name'] = FenSurface['Name'][i]
		if FenSurface['SurfaceType'][i] == 'Window':
			obj['Leakage Component Name'] = 'Janela'
			obj['Window/Door Opening Factor, or Crack Factor'] = '@@openfac@@'
			obj['Ventilation Control Mode'] = 'ZoneLevel'
		else:
			obj['Leakage Component Name'] = 'Porta'
			obj['Window/Door Opening Factor, or Crack Factor'] = 1           #Porta nao abre
			obj['Ventilation Control Mode'] = 'novent'
		#obj['Ventilation Control Zone Temperature Setpoint Schedule Name'] = 'TempVentilacao'
		#obj['Venting Availability Schedule Name'] = ''
		idf.add(obj)

	obj = IDF._create_datadict('Building')
	obj['Name'] = output #(str(pedireito)+'-'+str(bwidth)+'-'+str(bwidth)+'-'+str(wwr)+'-'+str(jandorm_parlat)+'-'+str(jansala_par4)+'-'+str(jansala_par3)+'-'+str(jansala_par2)+'-'+str(uh)+'-'+str(Adiabatic))
	...
	obj = IDF._create_datadict('AirflowNetwork:SimulationControl')
	obj['Name'] = 'VentNat-'+output
	obj['AirflowNetwork Control'] = 'MultizoneWithoutDistribution'
	obj['Height Selection for Local Wind Pressure Calculation'] = Cp
	if blengthratio > 1:
		obj[10] = (azimute)%180
	else:
		obj[10] = (azimute+90)%180
	obj[11] = bldgRatio
	idf.add(obj)
...
def sampleGen(file = 'sample.csv', path='C:\\Users\\Marcelo\\Documents\\'):
	...
	for line in lines:
		if condition == True:
			...
			print(output)
			#print(pedireito, bwidth, blengthratio, wwr, jandorm_parlat, jansala_par4, jansala_par3, jansala_par2, uh, adiabaticsala_par2, adiabaticsala_par4,output , path)
			out = main(pedireito, bwidth, blengthratio, wwr, jandorm_parlat, jansala_par4, jansala_par3, jansala_par2, uh, adiabaticsala_par2, adiabaticsala_par4, tamanhoprojecao, hpav, hjan, azimute, output , path)
